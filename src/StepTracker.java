...
public class StepTracker {
    ...
    void addSteps(){
        ...
        if (stepInfo.containsKey(month)) {//если месяц создан добавляем день
            ...
            monthCheck.add(day-1,steps);//Заменяем значение
            //for (Integer sss : monthCheck) {
    ...
    void getReport(){
        ...
        if (!(stepInfo.containsKey(reportMonth))){
            addMonth(reportMonth);
        }
        for (int i = 0; i < monthCheck.size(); i++) {
            if (monthCheck.get(i) != 0) {
                System.out.println((i + 1) + "день :" + monthCheck.get(i));
    ...
    void createMonth (String createMonth){
        String addMonth = createMonth;
        ArrayList<Integer> addMonth = new ArrayList<>();
        for (int i = 0; i < 30; i++) {//добавляем нули во все дни месяца
            addMonth.add(0);
        }
    }
